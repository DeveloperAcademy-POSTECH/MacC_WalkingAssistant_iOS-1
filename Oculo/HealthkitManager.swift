...
class HealthKitManager {
    ...
    private let healthStore = HKHealthStore()  /// 헬스 스토어 객체
    ...
    // MARK: healthKitManager의 init 메서드
    init(){
        requestAuthorizationAndRetrieveWalkingStepLength()
    }

    // MARK: 메서드
    private func calPeriod(period: Period) -> Date {  /// 분기 계산 메서드
            ...
            if error != nil {
                ...
                if success {
                    self.retrieveWalkingStepLength(period: self.periodToRetrieve) { (distance) in
                        self.walkingStepLength = Int(distance * 100)  /// 반환값 단위가 미터미므로 *100을 곱해 센티미터 단위로 변환하고 소숫점은 절사
                    }
                } else {
