...
class HealthkitManager {
    ...
    private let defalutWalkingStepLength: Int = 70// 한국인 평균 보폭 http://kpha.or.kr/board/view.php?p_pkid=6513&p_mid=16&p_mbs=01-06-02&p_code=&p_sdesc=&p_stype=&nowpage=&movepage=&p_soption1=
    ...
    private func calPeriod(period : Period) -> Date { // 분기 계산메서드
        ...
        switch period {
        case .day:
            return Calendar.current.date(byAdding: .hour, value: -24, to: currentDate)! // 현재시각으로부터 24시간전 시각 리턴
        case .month:
            return Calendar.current.date(byAdding: .month, value: -1, to: currentDate)! // 현재시각으로부터 1달전 시각 리턴
        case .quater:
            return Calendar.current.date(byAdding: .month, value: -3, to: currentDate)! // 현재시각으로부터 3달전 시각 리턴
        case .half:
            return Calendar.current.date(byAdding: .month, value: -6, to: currentDate)! // 현재시각으로부터 6달전 시각 리턴
        case .year:
            return Calendar.current.date(byAdding: .year, value: -1, to: currentDate)! // 현재시각으로부터 1년전 시각 리턴
        }
    }
    ...
    public func retrieveWalkingStepLength(period : Period, completion : @escaping (Double) -> Void) { // 건강앱의 보폭데이터를 받아오는 메소드
        ...
        let currentDate = Date() //현재시각
        let startDate = calPeriod(period: period)//시작시각
        let predicate = HKQuery.predicateForSamples(withStart: startDate, end: currentDate, options: .strictStartDate)//현재시각으로부터 지금시각까지로 기간설정
        ...
        let query = HKStatisticsQuery(quantityType: walkingStepLengthType, quantitySamplePredicate: predicate, options: .discreteAverage) { (_,result,error ) in
            ...
            stride = everage.doubleValue(for: HKUnit.meter())
